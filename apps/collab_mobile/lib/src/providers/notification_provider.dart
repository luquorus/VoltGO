...
class NotificationNotifier extends StateNotifier<NotificationState> {
  ...
  /// Get the API client
  CollaboratorMobileApiClient get _api => ref.read(apiClientFactoryProvider)!.collabMobile;
  ...
  Future<void> loadNotifications({bool refresh = false}) async {
    ...
    try {
      final response = await _api.getNotifications(
        category: state.categoryFilter == NotificationCategory.ALL ? null : state.categoryFilter.value,
...
final unreadNotificationCountProvider = FutureProvider<int>((ref) async {
  ...
  try {
    return await factory.collabMobile.getUnreadNotificationCount();
  } catch (e) {
    return 0;
  }
});
