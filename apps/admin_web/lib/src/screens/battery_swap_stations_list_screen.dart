...
class BatterySwapStationsListScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<BatterySwapStationsListScreen> createState() =>
      _BatterySwapStationsListScreenState();
}

class _BatterySwapStationsListScreenState
    extends ConsumerState<BatterySwapStationsListScreen> {
          ...
          Expanded(
            child: Card(
              ...
              child: stationsAsync.when(
                ...
                loading: () =>
                    const LoadingState(message: 'Loading battery swap stations...'),
  ...
  Widget _buildHeader(
      ...
      ThemeData theme,
      AsyncValue<PaginationResponse<BatterySwapStation>> stationsAsync) {
    if (isMobile(context)) {
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          stationsAsync.when(
            data: (response) => Text('Battery Swap Stations',
                style: theme.textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
            loading: () => Text('Battery Swap Stations',
                style: theme.textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
            error: (_, __) => Text('Battery Swap Stations',
                style: theme.textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
          ),
            ...
            children: [
              Expanded(
                child: OutlinedButton.icon(
                  onPressed: () =>
                      context.push('/battery-swap-stations/import-csv'),
              ...
              Expanded(
                child: ElevatedButton.icon(
                  onPressed: () =>
                      context.push('/battery-swap-stations/create'),
          ...
          suffixIcon: search != null && search.isNotEmpty
              ? IconButton(
                  ...
                  onPressed: () {
                    ...
                    ref.read(batterySwapStationsSearchProvider.notifier).state =
                        null;
  ...
  Widget _buildStationsTable(BuildContext context, ThemeData theme,
      WidgetRef ref, PaginationResponse<BatterySwapStation> response) {
  ...
  Widget _buildStationCard(BuildContext context, ThemeData theme,
      WidgetRef ref, BatterySwapStation station) {
    return InkWell(
      onTap: () => context.push('/battery-swap-stations/${station.id}'),
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(
              ...
              runSpacing: 8,
              children: [
                _buildInfoChip(theme, Icons.battery_full,
                    station.totalBatteries != null
                        ? '${station.availableBatteries ?? '?'}/${station.totalBatteries}'
                        : 'N/A'),
                _buildInfoChip(theme, Icons.ev_station,
                    station.totalPiles?.toString() ?? 'N/A'),
                if (station.trustScore != null)
                  _buildInfoChip(theme, Icons.star,
                      station.trustScore.toString()),
                _buildInfoChip(theme, Icons.pending_actions,
  ...
  void _showDeleteDialog(
      BuildContext context, WidgetRef ref, BatterySwapStation station) {
    showDialog(
      context: context,
